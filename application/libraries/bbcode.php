...
//
//   Class slightly modified to fit iDocsy uses.
...
class BBCode {
  ...
  private static $simple_search = array(
    ...
    '/\[center\](.*?)\[\/center\]/is',
    '/\[font\=((?=")"(.*?)"|([^"]*))\](.*?)\[\/font\]/is',
    '/\[align\=((?=")"(left|center|right)"|(left|center|right))\](.*?)\[\/align\]/is',
    ...
    // Bootrstap specific
    '/\[tooltip\=((?=")"(.*?)"|([^"]*))\](.*?)\[\/tooltip\]/is',
    '/\[tooltip\=((?=")"(.*?)"|([^"]*)) placement\=((?=")"(top|bottom)"|(top|bottom))\](.*?)\[\/tooltip\]/is',
    '/\[icon\=((?=")"(.*?)"|([^"]*)) ?\/\]/is',
    '/\[label\](.*?)\[\/label\]/is',
    '/\[label=((?=")"(.*?)"|([^"]*))\](.*?)\[\/label\]/is',
    '/\[badge\](.*?)\[\/badge\]/is',
  ...
  private static $simple_replace = array(
    ...
    '<span style="font-size: $2$3;">$4</span>',
    '<span style="color: $2$3;">$4</span>',
    '<div style="text-align: center;">$1</div>',
    '<span style="font-family: $2$3;">$4</span>',
    '<div style="text-align: $2$3;">$4</div>',
    ...
    '<img src="$1" alt="" />',
    '<img height="$3$5" width="$2$4" alt="" src="$6" />',
    ...
    '<blockquote><strong>$2$3 wrote:</strong> $4</blockquote>',   // you can also use: '<div class="quote"><strong>$1 wrote:</strong> $2</div>
    '<pre class="prettyprint linenums lang-lua">$1</pre>',
    '<pre class="prettyprint linenums lang-$2$3">$4</pre>',

    // Bootstrap specific
    '<a href="#" rel="tooltip" data-original-title="$2$3">$4</a>',
    '<a href="#" rel="tooltip" data-original-title="$2$3" data-placement="$4$5">$6</a>',
    '<i class="icon-$2$3"></i>',
    '<span class="label">$1</span>',
    '<span class="label label-$2$3">$4</span>',
    '<span class="badge">$1</span>',
    '<span class="badge badge-$2$3">$4</span>',
  ...
  private static $lineBreaks_search = array(
    ...
    private static $lineBreaks_replace = array(
      ...
      "'[list='.str_replace('\\\"', '\"', '$1').']'.str_replace('<br />', '', '$2').'[/list]'",
      "[/list]",

      // [list]
      //"\n[list$1]",         // uncomment to remove <br /> before the tag
      "'[list]'.str_replace('<br />', '', '$1').'[/list]'",
      "[/list]",

      // [code]
      //"\n[code]",           // uncomment to remove <br /> before the tag
      "'[code='.str_replace('\\\"', '\"', '$1').']'.str_replace('<br />', '', '$2').'[/code]'",
      "[/code]",

      // [code]
      //"\n[code]",           // uncomment to remove <br /> before the tag
      "'[code]'.str_replace('<br />', '', '$2').'[/code]'",
      "[/code]",
  ...
  */
  public static function parse($string) {
    ...
    $s = preg_replace('/\[url\=((?=")"(http:\/\/)?([^"]+?)"|(http:\/\/)?([^"]+?))\](.*?)\[\/url\]/i', '[url="http://$3$5"]$6[/url]', $s);
    ...
    $s = preg_replace('/\[list\=((?=")"(disc|circle|square|decimal|decimal-leading-zero|lower-roman|upper-roman|lower-greek|lower-alpha|lower-latin|upper-alpha|upper-latin|hebrew|armenian|georgian|cjk-ideographic|hiragana|katakana|hiragana-iroha|katakana-iroha|none)"|(disc|circle|square|decimal|decimal-leading-zero|lower-roman|upper-roman|lower-greek|lower-alpha|lower-latin|upper-alpha|upper-latin|hebrew|armenian|georgian|cjk-ideographic|hiragana|katakana|hiragana-iroha|katakana-iroha|none))\](.*?)\[\/list\]/sie',
                '"<ol style=\"list-style-type: $2$3;\">\n".self::process_list_items("$4")."\n</ol>"', $s);
